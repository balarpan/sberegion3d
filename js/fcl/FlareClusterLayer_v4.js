...
define(["require", "exports", "esri/layers/GraphicsLayer", "esri/geometry/Extent",  "esri/symbols/SimpleMarkerSymbol", "esri/symbols/TextSymbol", "esri/symbols/SimpleLineSymbol", "esri/Color", "esri/core/watchUtils", "esri/geometry/support/webMercatorUtils", "esri/Graphic", "esri/geometry/Point", "esri/geometry/Multipoint", "esri/geometry/Polygon", "esri/geometry/geometryEngine", "esri/geometry/SpatialReference", "esri/views/2d/engine/graphics/GFXObject", "esri/views/2d/engine/graphics/Projector", "esri/core/accessorSupport/decorators", "dojo/on", "dojox/gfx", "dojo/dom-geometry", "dojo/dom-construct", "dojo/query", "dojo/dom-attr", "dojo/dom-style"], function (require, exports, GraphicsLayer, Extent, SimpleMarkerSymbol, TextSymbol, SimpleLineSymbol, Color, watchUtils, webMercatorUtils, Graphic, Point, Multipoint, Polygon, geometryEngine, SpatialReference, GFXObject, Projector, asd, on, gfx, domGeom, domConstruct, query, domAttr, domStyle) {
    "use strict";
    ...
    var FlareClusterLayer = (function (_super) {
        ...
        function FlareClusterLayer(options) {
            ...
            _this.isZoomOnClick = !!options.isZoomOnClick || false;
        ...
        FlareClusterLayer.prototype._addViewEvents = function (layerView) {
            ...
            if (!v.fclPointerMove) {
                ...
                v.fclPointerDown = v.on("pointer-down", function (evt) { return _this._viewPointerDown(evt); });
            }
        };
        ...
        FlareClusterLayer.prototype._viewPointerMove = function (evt) {
            ...
            if (this._activeCluster) {
                var bbox = domGeom.position( this._activeCluster.clusterGroup.rawNode, true );
                if (bbox) {
                    if (mousePos.x >= bbox.x && mousePos.x <= (bbox.x + bbox.w) && mousePos.y >= bbox.y && mousePos.y <= (bbox.y + bbox.h))
                        return;
                }
            }
            var v = this._activeView;
            if (!this._activeView.updating && this._activeView.ready){
                this._activeView.hitTest(evt).then(function (response) {
        ...
        FlareClusterLayer.prototype._viewPointerDown = function (evt) {
            if ( !this.isZoomOnClick )
            return;

            var arrayMin = function(arr, ind) {
            ...
            this._activeView.hitTest(evt).then(function (response) {
                ...
                for (var i = 0, len = graphics.length; i < len; i++) {
                    ...
                    if (g && (g.attributes.clusterId != null && !g.attributes.isClusterArea)) {
                        var cluster = _this._clusters[g.attributes.clusterId];
                        var pnts = cluster.gridCluster.points;
                        var ext = new Extent( {xmin:arrayMin(pnts,0), xmax:arrayMax(pnts,0), ymin:arrayMin(pnts,1), ymax:arrayMax(pnts,1), spatialReference:{wkid:_this.spatialReference.wkid}} );
                        if( !ext.width || !ext.height) {
                            //In case when we have one point or all points in same place we need to enlarge extend a liitle
                            var dX = _this.spatialReference.isGeographic ? 0.00085 : 150;
                            var dY = _this.spatialReference.isGeographic ? 0.00085 : 150;
                            ext = new Extent( {xmin:ext.xmin - dX, xmax:ext.xmax + dX, ymin:ext.ymin - dY, ymax:ext.ymax + dY, spatialReference:{wkid:_this.spatialReference.wkid}} );
                            // ext.xmin -= - dX; ext.xmax += dX;
                            // ext.ymin -= - dY; ext.ymax += dY;
                        }
                        ext.expand(3)
                        console.log( ext, Polygon.fromExtent(ext) );
                        // _this._activeView.extent = ext;
                        // _this._activeView.goTo( Polygon.fromExtent(ext) )
                        _this._activeView.goTo( ext )
                    }
                }
            });
        ...
        FlareClusterLayer.prototype._getMousePos = function (evt) {
            ...
            var rect = domGeom.position(container, true)
            return {
                x: evt.x - rect.x,
                y: evt.y + rect.y
            };
